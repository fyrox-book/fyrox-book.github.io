...
1. `Add Animation` - adds a new empty animation with the name from the text box at the left to the animation player.
2. `Import Animation` - starts animation importing process. See [Animation Importing](#animation-importing) section
   for more info.
3. `Animation Selector` - allows you to switch currently edited animation.
...
6. `Delete Animation` - deletes a currently selected animation, tries to select last animation from the list if
   possible.
7. `Duplicate Animation` - clones a currently selected animation.
8. `Root Motion` - open root motion settings. See [Root Motion](#root-motion) section for more info.
...
11. `Preview Switch` - enables or disables animation preview. See [`Preview Mode`](#preview-mode) section for more info.
12. `Play/Pause` - plays or pauses a currently selected animation (allowed only in the preview mode).
13. `Stop` - stops a currently selected animation (allowed only in the preview mode).
14. `Animation Speed` - sets a new playback speed of a currently selected animation.
15. `Time Slice Start` - a time range (in seconds) which defines start time of a currently selected animation. The
    range is highlighted in the curve editor.
16. `Time Slice End` - a time range (in seconds) which defines end time of a currently selected animation. The
    range is highlighted in the curve editor.
...
9. `Add Position Track` - similar to `8`, but automatically binds to local position of a node.
10. `Add Scale Track` - similar to `8`, but automatically binds to local scale of a node.
11. `Add Rotation Track` - similar to `8`, but automatically binds to local rotation of a node.
12. `Add Key` - captures the state of all properties for the selected object. Only added properties will be captured.
...
You can cancel property binding at any time by clicking `Cancel` in any of the windows. Keep in mind that you can
animate
